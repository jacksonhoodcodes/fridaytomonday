...
const primaryNav: Array<{ href: Route; label: string; align: string }> = [
  { href: '/logs', label: 'Tue–Thu Log', align: 'text-center sm:text-left' },
  { href: '/sprints', label: 'Fri–Mon Grind', align: 'text-center' },
  { href: '/portfolio', label: 'Portfolio', align: 'text-center sm:text-right' }
];
        ...
        <div className="mx-auto w-full max-w-5xl px-4">
          ...
          <nav className="border-t border-zinc-200/70 py-3">
            <ul className="grid grid-cols-1 gap-2 text-sm font-medium text-zinc-600 sm:grid-cols-3 sm:items-center">
              {primaryNav.map((item) => (
                <li key={item.href} className={item.align}>
                  <Link href={item.href} className="inline-block rounded-full px-3 py-1.5 hover:bg-zinc-100 hover:text-zinc-900">
                    {item.label}
                  </Link>
                </li>
              ))}
